...
async function testWizard() {
  console.log('🧙‍♂️ ZKP-Estate Input Wizard Test');
  ...
  if (inputs.age < mockPolicy.minAge) {
    warnings.push(`⚠️  Age ${inputs.age} is below minimum required age ${mockPolicy.minAge}`);
  }
  
  const requiredIncome = mockPolicy.incomeMul * mockPolicy.rentWei;
  if (inputs.income < requiredIncome) {
    warnings.push(`⚠️  Income ${inputs.income} is below required ${requiredIncome} (${mockPolicy.incomeMul}x rent)`);
  }
  ...
  // Test nargo execute
  try {
    console.log('🔮 Testing nargo execute...');
    const circuitsDir = join(process.cwd(), 'circuits', 'eligibility');
    const output = execSync('nargo execute', { 
      cwd: circuitsDir,
      encoding: 'utf8'
    });
    console.log('✅ nargo execute successful');
  } catch (error) {
    console.error('❌ nargo execute failed:', error);
  }
