...
async function main() {
  ...
  // Default values
  let proofFile = "artifacts/proofs/sample.json";
  let policyId: number | null = null;
  let walletIndex = 0; // Use first account by default
...
Examples:
  ...
  const publicInputsBigInt = proofData.publicInputs.map((input: string) => BigInt(input));
  ...
  try {
    ...
    const nullifierUsed = await eligibilityGate.isNullifierUsed(nullifier);
