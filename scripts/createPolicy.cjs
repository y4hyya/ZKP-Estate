...
async function main() {
  ...
  let minAge = parseInt(process.env.MIN_AGE) || 18;
  let incomeMul = parseInt(process.env.INCOME_MUL) || 3;
  let rentWei = ethers.parseEther(process.env.RENT_ETH || "1.0");
  let needCleanRec = process.env.CLEAN_RECORD ? process.env.CLEAN_RECORD.toLowerCase() === "true" : true;
  let deadline = Math.floor(Date.now() / 1000) + (parseInt(process.env.DEADLINE_DAYS) || 30) * 86400;
